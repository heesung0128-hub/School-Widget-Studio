        ...
        {activeTab === 'school' && (
          <div className="space-y-4">
            ...
            <div className="p-3 bg-slate-800/60 rounded-xl border border-slate-700/70">
              <div className="text-xs text-slate-400 mb-1">현재 적용된 학교</div>
              <div className="flex items-center justify-between">
                <div>
                  <h4 className="text-sm font-bold text-blue-400">{config.school.schoolName}</h4>
                  <p className="text-xs text-slate-400 mt-0.5">{config.school.officeName} ({config.school.officeCode}) • 코드: {config.school.schoolCode}</p>
                </div>
            ...
            <form onSubmit={handleSearchSchool} className="space-y-2">
              ...
              <div className="flex gap-2">
                <input
                  ...
                  className="flex-1 px-3 py-2 text-xs rounded-xl bg-slate-800 border border-slate-700 text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
                />
                <button
                  type="submit"
                  disabled={isSearching}
                  className="px-4 py-2 bg-blue-600 hover:bg-blue-500 disabled:opacity-50 text-white text-xs font-bold rounded-xl flex items-center gap-1.5 transition-colors"
                >
            ...
            {searchMessage && (
              <p className="text-xs text-amber-400">{searchMessage}</p>
            )}

            {/* Search Results List */}
            {searchResults.length > 0 && (
              <div className="space-y-1.5 max-h-48 overflow-y-auto pr-1">
                <div className="text-xs text-slate-400">검색 결과 ({searchResults.length}건):</div>
                {searchResults.map((sch) => (
                  ...
                  >
                    ...
                    <span className="text-xs text-blue-400 group-hover:underline">선택하기</span>
                  </button>
                ))}
              </div>
            )}
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    ...
                    className="w-full px-3 py-1.5 text-xs rounded-lg bg-slate-800 border border-slate-700 text-white focus:outline-none focus:border-blue-500"
                  />
        ...
        {activeTab === 'ddays' && (
          <div className="space-y-4">
            ...
            <form onSubmit={handleAddDDay} className="p-3 bg-slate-800/60 rounded-xl border border-slate-700/60 space-y-2">
              <div className="text-xs font-bold text-slate-300">새 D-Day 추가</div>
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                <input
                  ...
                  className="px-3 py-1.5 text-xs rounded-lg bg-slate-800 border border-slate-700 text-white placeholder:text-slate-500 focus:outline-none focus:border-blue-500"
                />
                <input
                  type="date"
                  value={newDDayDate}
                  onChange={(e) => setNewDDayDate(e.target.value)}
                  className="px-3 py-1.5 text-xs rounded-lg bg-slate-800 border border-slate-700 text-white focus:outline-none focus:border-blue-500"
                />
              </div>
              <button
                type="submit"
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between text-xs text-slate-400">
                ...
                <span className="text-[11px] text-slate-500">화살표(↑↓)로 위젯 표시 순서 변경</span>
              </div>
              {config.ddays.length === 0 ? (
                <p className="text-xs text-slate-500 py-3 text-center">등록된 D-Day가 없습니다.</p>
              ) : (
                config.ddays.map((d, idx) => (
                  ...
                  >
                    <div className="flex items-center gap-2 flex-1 min-w-0">
                      <div className="w-5 h-5 rounded-md bg-slate-700/60 flex items-center justify-center text-[10px] font-bold text-slate-400 shrink-0">
                        {idx + 1}
                      </div>
                      <div className="min-w-0 flex-1">
                        <div className="text-xs font-bold text-slate-200 truncate">{d.title}</div>
                        <div className="text-[11px] text-slate-400">목표일: {d.targetDate}</div>
                      </div>
                    </div>
        ...
        {activeTab === 'timetable' && (
          <div className="space-y-4">
            ...
            <div className="flex flex-wrap items-center justify-between gap-2 p-2.5 bg-slate-800/60 rounded-xl border border-slate-700/60">
              <span className="text-xs font-semibold text-slate-300">시간표 빠른 프리셋:</span>
              <div className="flex gap-1.5">
                <button
                  type="button"
                  onClick={() => applyTimetablePreset('high-teacher')}
                  className="px-2 py-1 text-[11px] bg-slate-700 hover:bg-slate-600 text-slate-200 rounded-md font-medium transition-colors"
                >
                  고교 교사용
                </button>
                <button
              ...
              {(['월', '화', '수', '목', '금'] as const).map((day) => {
                ...
                return (
                  <div key={day} className="p-2.5 bg-slate-800/80 rounded-xl border border-slate-700">
                    <div className="text-xs font-bold text-blue-400 mb-2">{day}요일 시간표</div>
                    <div className="grid grid-cols-7 gap-1">
                      {dayObj.periods.map((subject, pIdx) => (
                        <div key={pIdx} className="space-y-1">
                          <label className="text-[10px] text-slate-400 block text-center">
                            {pIdx + 1}교시
                          </label>
                          <input
                            type="text"
                            ...
                            placeholder="-"
                            className="w-full px-1 py-1 text-[11px] text-center rounded bg-slate-900 border border-slate-700 text-slate-200 focus:outline-none focus:border-blue-500"
                          />
                        </div>
                      ))}
                    </div>
        ...
        {activeTab === 'style' && (
          <div className="space-y-4">
            ...
            <div className="pt-2 border-t border-slate-800 flex items-center justify-between">
              <div>
                <div className="text-xs font-bold text-slate-200">항상 위에 표시 (Topmost)</div>
                <div className="text-[11px] text-slate-400">다른 작업 창 위에 위젯을 항상 고정합니다.</div>
              </div>
              <input
                type="checkbox"
